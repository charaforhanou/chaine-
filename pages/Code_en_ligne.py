...
class BinaryTransmissionApp:
    ...
    def apply_Miller(self, binary_sequence):
        ...
        for i, bit in enumerate(binary_sequence):
            if bit == 0:
                if prev_bit == 0:
                    miller_sequence.extend([0, 1])  # Transition at the end of the bit
                else:
                    miller_sequence.extend([0, 0])  # No transition
            elif bit == 1:
                miller_sequence.extend([1, 0])  # Transition in the middle of the bit
            prev_bit = bit
